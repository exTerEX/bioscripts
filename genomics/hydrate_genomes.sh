...
show_help() {
  ...
  echo "Rehydrate a dehydrated NCBI datasets zip archive."
  echo ""
  echo "Options:"
  echo "  -h, --help          Display this help message"
  echo ""
  echo "  -i <input>          Dehydrated NCBI datasets zip file to rehydrate"
  echo ""
  echo "  -o <output>         Output directory for rehydrated genomic data"
  echo ""
...
if [[ ! -v input_file || ! -v output_directory ]]; then
...
unzip -o "$input_file" -d "$output_directory"
...
datasets rehydrate "${api_key_args[@]}" --directory "$output_directory"
