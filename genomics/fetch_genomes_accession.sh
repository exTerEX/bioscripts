...
show_help() {
  ...
  echo "Download genomes by GenBank/RefSeq accession using NCBI datasets."
  echo ""
  echo "Options:"
  echo "  -h, --help          Display this help message"
  echo ""
  echo "  -i <input>          Input file with one GenBank/RefSeq accession per line"
  echo ""
...
if [[ ! -v input_list || ! -v output_directory ]]; then
    echo "Error: Both -i <input> and -o <output> are required." >&2
    exit 1
fi

if [[ ! -f "$input_list" ]]; then
    echo "Error: Input file not found: $input_list" >&2
    exit 1
fi
...
# Load the accession values as a comma-separated string
accession=$(paste -sd, "$input_list")
